...
class DM_Bootstrap
{
    ...
    public function admin_enqueue($hook)
    {
        ...
        $roles = array_map(function($r){
            return translate_user_role($r['name']);
        }, $wp_roles->roles);
    ...
    {
        if (get_option('dm_document_version', '0.0.0') != DM_VERSION || DM_DEBUG) {
            (new DM_Document_Model())->createTable()->updateTable();
            (new DM_Category_Model())->createTable()->updateTable();
            update_option('dm_document_version', DM_VERSION);            
        }
    }
